...
{
  ros::init(argc, argv, "uwb_driver_a");
  ros::NodeHandle n;
  ros::Publisher uwb_pub = n.advertise<sensor_msgs::Range>("uwb_data_a", 1000);
  ...
  printf("module 1 - step 1 Initialize the interface to the RCM.\n");
  // initialize the interface to the RCM
  if (rcmIfInit(rcmIfIp, ip) != OK)
  {
    printf("module 1 - Initialization failed.\n");
    exit(0);
  }
  printf("module 1 - step 2 Set sleep mode idle.\n");
  if (rcmSleepModeSet(RCRM_SLEEP_MODE_IDLE) != 0)
  {
    printf("module 1 - Time out waiting for sleep mode set 1.\n");
    exit(0);
  }
  printf("module 1 - step 3 Execute Built-In Test.\n");
  // execute Built-In Test - verify that radio is healthy
  if (rcmBit(&status) != 0)
  {
    printf("module 1 - Time out waiting for BIT.\n");
    
    exit(0);
  }
  if (status != OK)
  {
    printf("module 1 - Built-in test failed - status %d.\n", status);
    exit(0);
  }
  printf("module 1 - step 4 Set sleep mode active.\n");
  if (rcmSleepModeSet(RCRM_SLEEP_MODE_ACTIVE) != 0)
  {
    printf("module 1 - Time out waiting for sleep mode set 2.\n");
    exit(0);
  }
  printf("module 1 - Init done.\n");
  while(ros::ok()){
